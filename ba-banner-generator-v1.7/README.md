...
6. **Download** — when done, two buttons appear:
   - **⬇ Preview Bundle** — single self-contained `.html` file with all banners inlined; open directly in any browser, no extraction needed (QA-ready)
   - **⬇ Deliverables ZIP** — each banner size as its own `.zip`, structured `CampaignType/Funnel/CopyCode/` (for ad platform trafficking)
